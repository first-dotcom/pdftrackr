...
export const normalizeIp = (req: Request, _res: Response, next: NextFunction) => {
  let ip = req.ip;
  ...
  (req as Request & { normalizedIp?: string }).normalizedIp = ip;
  next();
};
...
export const createRateLimit = (windowMs: number, max: number, message = "Too many requests") => {
